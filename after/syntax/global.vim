" Fold titles in bold:
syn match customCommentFoldTitle '\w.*\ze.*{{\(\){\d*$' contained containedin=.*Comment.*
...
" Conceal markers:
" setl conceallevel=2
syn match customConcealedMarkerStart '{{\(\){\d*$' contained containedin=.*Comment.* conceal cchar=⟪
